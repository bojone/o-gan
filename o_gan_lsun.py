...
imgs = glob.glob('../../lsun/church_outdoor/imgs/*.png')
...
def imread(f, mode='gan', crop_size=256):
    x = misc.imread(f, mode='RGB')
    height,width = x.shape[:2]
    crop_size = min([crop_size, height, width])
    height_idx = (height - crop_size + 1) // 2
    width_idx = (width - crop_size + 1 ) // 2
    x = x[height_idx: height_idx+crop_size,
